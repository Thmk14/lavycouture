...
$sql = "SELECT 
            cl.nom, cl.prenom, cl.lieu_habitation, cl.telephone,
            cmd.id_commande, cmd.statut_commande, cmd.date_liv_souhait,
            SUM(p.quantite * v.prix) AS total_commande,
            liv.statut_livraison, liv.date_livraison
        FROM commande cmd   
        JOIN client cl ON cmd.id_client = cl.id_client
        JOIN panier p ON p.id_client = cl.id_client
        JOIN vetement v ON v.id_vetement = p.id_vetement
        LEFT JOIN livraison liv ON liv.id_commande = cmd.id_commande
        WHERE cmd.statut_commande = 'prête'
        GROUP BY cmd.id_commande
        ORDER BY liv.date_livraison DESC";
...
if (isset($_GET['id']) && isset($_GET['statut'])) {
    ...
    if ($statut == 'livrée') {
        $update = $pdo->prepare("UPDATE livraison SET statut_livraison = 'livrée' WHERE id_commande = ?");
                ...
                <tbody>
                    <?php foreach ($commandes as $cmd): ?>
                        <tr>
                            ...
                            <td><?= number_format($cmd['total_commande'], 0, ',', ' ') ?> FCFA</td>
                            <td><?= htmlspecialchars($cmd['date_liv_souhait']) ?></td>
                            <td>
  <form method="POST" action="mise_date_liv.php" style="display:flex; align-items:center; justify-content:center; gap:5px;">
    ...
    <button type="submit" style="padding:5px 8px; font-size:14px; background:#a72872; color:#fff; border:none; border-radius:4px; cursor:pointer;">
      Modifier
                            ...
                            <td><?= htmlspecialchars($cmd['statut_commande']) ?></td>
                            <td><?= htmlspecialchars($cmd['statut_livraison'] ?? 'en attente') ?></td>
                            <td>
                                ...
                                <?php if (($cmd['statut_livraison'] ?? '') === 'livrée'): ?>
