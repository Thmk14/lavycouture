...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register'])) {
    ...
    if (!empty($name) && !empty($lastname) && !empty($email) && !empty($password) && !empty($telephone)) {
        ...
        $checkEmail = $pdo->prepare("SELECT id_couturier FROM couturier WHERE email = ?");
        ...
        if ($checkEmail->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("INSERT INTO couturier (nom, prenom, email, mot_de_passe, telephone) VALUES (?, ?, ?, ?, ?)");
            try {
                ...
                header("Location: listcouturier.php");
                exit();
...
<div class="container">
    ...
    <div class="form-box active" id="register-form">
        <form method="POST">
            <h2 class="h2">Ajouter couturier</h2>
